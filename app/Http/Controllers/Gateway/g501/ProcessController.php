...
class ProcessController extends Controller
{
    ...
    public static function process($deposit)
    {
        ...
        if ($data->btc_amo == 0 || $data->btc_wallet == "") {
            ...
            $invoice_id = $data->trx;
            $callback_url = route('g501') . "?invoice_id=" . $invoice_id . "&secret=" . $secret;
    ...
    public function ipn()
    {
        ...
        if ($data->btc_amo == $value_in_btc && $_GET['address'] == $data->btc_wallet && $_GET['secret'] == "PROPHECY" && $_GET['confirmations'] > 2 && $data->status == 0) {
